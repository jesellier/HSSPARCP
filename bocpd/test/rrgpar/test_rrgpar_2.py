...
class Test_RRGPAR(unittest.TestCase):
    ...
    def setUp(self) :
        ...
        m_tf = RRGPAR_TF( lrgp_tf,  n_dimension, noise_parameter)
        m_tf.setData(data)
        m_tf.initialize()
        m_tf._jitter = jitter
        ...
        m = RRGPAR(k,  n_dimension, noise_parameter )
        m._jitter = jitter
        m.setData(data)
        m.computeGradient()
        m.initialize()
    ...
    def test_global_grad(self):
        ...
        dnlml_tf = m_tf.grad_adjustment_factor * TensorMisc().pack_tensors(dnlml_tf )
