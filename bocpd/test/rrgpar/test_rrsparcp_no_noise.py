...
from bocpd.test.RRGPAR_tf import RRSPARCP_NO_NOISE_TF, GaussianLaplaceReducedRank_TF
...
class Test_RRSPARCP(unittest.TestCase):
    ...
    def setUp(self) :
        ...
        #MODEL1 = RRSPAR_TF
        k_tf = GaussianLaplaceReducedRank_TF(variance, lengthscales , n_features = 15, L = 3)
        m_tf =  RRSPARCP_NO_NOISE_TF(k_tf,  1, prior_parameter)
        m_tf.setData(data)
        m_tf.initialize()
        m_tf._jitter = jitter
        ...
        m =  RRSPARCP_no_noise(k, 1, prior_parameter)
        m._jitter = jitter
        m.setData(data)
    ...
    def test_SSE_update(self):
        ...
        for i in range(n+1):
            print("[rec, tf, np]:= :[{}".format(SSE_rec[i][0]) + ",{}".format(SSE_tf[i][0]) + ",{}".format(SSE_t[i]) + "]")
