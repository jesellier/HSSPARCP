...
from bocpd.GPAR.GPARCP import SPARCP
from bocpd.test.GPAR_tf import SPARCP_TF
...
class Test_SPARCP(unittest.TestCase):
    ...
    def setUp(self) :
        ...
        #MODEL1 = SPARCP_TF
        m_tf = SPARCP_TF(k_tf, 1, alpha, noise_scale)
        m_tf.setData(data)
        m_tf._jitter = jitter

        #MODEL2 =  SPARCP
        k = ConstantKernel(variance) * RBF(length_scale=lengthscales )
        m = SPARCP(k, 1, alpha, noise_scale)
        m.setData(data)
        m._jitter = jitter
        m.computeGradient()
    ...
    def test_logpdf_n(self):
        ...
        for i in range(len(logpdf)):
             ...
             #self.assertAlmostEqual(logpdf[i], logpdf_tf[i][0].numpy(), places=6)
    ...
    def test_logpdf_0(self):
        ...
        if m_tf.noise_trainable :
            tmp = np.append(dlogpdf_tf[:-2], 0)
            tmp = np.append(tmp, dlogpdf_tf[-1])
            dlogpdf_tf = tmp
        ...
        print("np_pdf:= : {}".format(logpdf[0][0]))
        self.assertAlmostEqual(logpdf[0][0], logpdf_tf[0][0].numpy(), places=7)
        
        print("")
        print("tf_grad:= : {}".format(np.array(dlogpdf_tf)))
        print("np_grad:= : {}".format(dlogpdf[0]))
    ...
    def test_prediction_n(self):
        ...
        for i in range(n+1):
              ...
              #self.assertAlmostEqual(mu_scp[i][0], mu_tf[i][0].numpy(), places=5)
              #self.assertAlmostEqual(mu_scp[i][0], mu[i], places=5)
